...
describe('computeTotals', () => {
  it('returns an empty array for no rows', () => {
    expect(computeTotals([], 'area')).toEqual([]);
  });
  ...
  it('ignores pending and drawing rows', () => {
    ...
    expect(computeTotals(rows, 'area')).toEqual([]);
  });
  ...
  it('sums a single unit', () => {
    ...
    expect(computeTotals(rows, 'area')).toEqual([{ unit: 'mm2', value: 150, count: 2 }]);
  });
  ...
  it('keeps mm2 and px2 as two separate totals, never merged', () => {
    ...
    const totals = computeTotals(rows, 'area');
  ...
  it('orders mm2 first, then px2, then other units alphabetically', () => {
    ...
    expect(computeTotals(rows, 'area').map((t) => t.unit)).toEqual(['mm2', 'px2', 'mm', 'px']);
  });
  ...
  it('reports the correct count per unit group', () => {
    ...
    expect(computeTotals(rows, 'area')[0].count).toBe(3);
  });
  ...
  it('sums floating-point values stably, deferring rounding to display', () => {
    ...
    expect(computeTotals(rows, 'area')[0].value).toBeCloseTo(0.3, 10);
  });
});
