...
| State restore (S-5.6)   | [`storedRows.ts`](../host-app/src/state/storedRows.ts) over [`storage.ts`](../host-app/src/services/storage.ts), [`restoreViewer`](../host-app/src/state/actions.ts#L64) on every announcement | `restore.ts` (readiness gate, re-add with the original uid), `ohif/metrics.ts` (`toGeometry`)                                                                |
