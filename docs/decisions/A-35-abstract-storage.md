...
generic service applies whatever schema it is handed and the rows' service owns its own. Rejected:
keeping one module and renaming it (the generic service would still import `Row`); a generic
storage class with methods per key (a factory for two functions); a `useStoredRows` hook owning
the reducer (the author: the reducer is not to be mixed into persistence, and a persistence hook
without the state has nothing to return — §0.1); and `state/` as its home (it is I/O, not state).
