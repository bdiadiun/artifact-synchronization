...
- **One implementation, one interface.** The end of the channel is described by
  `{ peerOrigin, accept, readyOn?, peerWindow? }` and offers `{ send, on, getState }` on both sides
  (since [A-34](A-34-two-applications-one-channel-hook.md): the end is created when the package's
  module loads and reached through `useChannel(options)`; this record first had a
  `createChannel` factory returning `{ send, onMessage, getState, subscribe, dispose }`). `accept` is the
  ...
  queue in order before the application's handler sees the message, so the handler can answer a
  peer that announced itself again (a viewer reload) with plain sends. The state is `{ ready,
queued }` (amended 2026-09-23 by [A-33](A-33-review-of-the-boundaries-refactor.md): the
  `announcements` counter went with the effect that reacted to it). Without `readyOn` the end is
  ready at once. The viewer names its peer window (`window.parent`, only when it is embedded); the
  host names none and learns it from the source of the first message it accepts, its
  `VIEWER_READY` — so the form never needs a reference to the iframe.
...
- **No `DEACTIVATE_TOOL` on unmount, no dispose on `pagehide`.** The host application has one
  ...
  extension's subscriptions and its armed row live in one React effect and die with its mount
  ([A-32](A-32-react-bridge.md); the `pagehide` listener this record first allowed went with it).
