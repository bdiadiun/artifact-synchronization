...
| `src/services/`   | `storage` (`readStorage(key, schema)` / `writeStorage(key, value)`: sessionStorage, defensively, knowing nothing about studies or rows — A-35) and `storedRows` (the rows' I/O adapter over it: the envelope, the key, `loadRows` / `saveRows`); the channel itself lives in the published package and is reached through its `useChannel` (A-34). |
