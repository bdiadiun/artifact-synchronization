...
| `src/form/`       | The row model and everything around it: `rows` (types, reducer, lookups), `rowActions`, `viewerEventHandlers`, `storage` (with its two hooks), `totals`, `format`, `useScoringForm`. |
...
`packages/contract` (the wire contract, four flat files), `packages/channel` (`host/`, `viewer/`,
`shared/`), `packages/viewer-bridge` (`commands/`, `events/`, `ohif/`), `packages/viewer-adapter`;
