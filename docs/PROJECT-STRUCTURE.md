...
| `src/form/`       | The row model and everything around it: `rows` (types, reducer, lookups), `rowActions`, `viewerEventHandlers`, `useSessionStorage`, `storage` (`useStoredForm`), `totals`, `format`, `useScoringForm`. |
