...
| F-07 | Form rows: add, statuses, row IDs | C-4.3.1, C-4.3.2, C-4.3.7, Q-3 | F-02 | 3 `feat: activate ellipse from form` | review | "Add measurement" creates rows with unique IDs and status `Pending`; any number of rows can be added. |
| F-08 | Activate / deactivate tool from a row | C-4.3.3, C-4.4.1, Q-3, A-4, P-7 | F-05, F-06, F-07 | 3 | review | "Activate" switches the row to `Drawing…` and the viewer to `EllipticalROI`; cancel sends `DEACTIVATE_TOOL`, row returns to `Pending`, viewer returns to the default tool. |
