...
| F-67 | Boundaries as schemas                                                                                                                             | A-13, D-2, Q-7                                                  | F-66             | 61    | approved | All tests pass; no `.on(` call on a channel; no `as Partial<Metrics>` in host-app; no `typeof` guard beside a schema in the extension; a metric with an unknown key is refused by the guard; the browser scenario passes on the linked working tree.                                                                                                                                                                                                                                                      |
...
graph TD
  ...
  F67["F-67 Boundaries as schemas"]
...
### F-67 Boundaries as schemas

The two remaining boundaries follow the contract's pattern (A-28): OHIF's measurement object is checked by one zod schema in the extension, the form's Row is a schema the stored row is derived from, metric keys are an enum so no cast is needed to read a metric, and toMetrics reads the contract's tool-to-metric map. API without a caller is removed: channel.on, the viewer end's state, the disposer set, the hostOrigin option; OHIF services are typed as required with one start-up check. Curried factories left in the bridge and in MeasurementRow are plain functions.

Canon: A-13, D-2, Q-7. Depends on: F-66. Slice 61, status `approved`.
