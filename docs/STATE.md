...
| Field          | Value                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current slice  | 64 `refactor: a tool yields a list of metrics` (decision A-40) — `METRIC_KEYS_BY_TOOL` is a list per tool, the extension reads it through two flat tables (`UNITS`, `UNIT_FIELD`); contract 0.1.0, channel 0.0.10, scoring-viewer 0.1.2, ohif-extension-loader 0.1.2; after the merge the publish workflow runs, then a fork PR takes loader 0.1.2 and a pin PR records the fork commit |
| Gate           | 2 — implementation reviewed and verified (typecheck, lint, format, tests, end-to-end scenario 13/13 on the linked tree); waiting for the author's approval to commit and open the PR                                                                                                                                                                                                    |
...
| Next slice     | none — the repository was submitted on 2026-09-23; slice 64 came out of the defence rehearsal (the P-8 question) and is the last change; anything after it (the study-change decision in Follow-ups, more tests) is optional and would start at gate 1                                                                                                                                  |
...
- After slice 16: 227 comment lines vs 3 499 code lines (6%); rules in CONVENTIONS §8. Since slice 61 (2026-09-23) the
  packages and the form carry no comments except one DefinePlugin directive; the reasons are in `docs/notes/viewer-internals.md`
  and `docs/notes/form-internals.md`.

## Comment baseline before slice 16

- 937 comment lines vs 3 499 code lines (21%); worst: `focus.ts` 57%, `getCustomizationModule.tsx` 52%, `removals.ts` 48%, `ui-strings.ts` 45%, `session.ts` 41%.
