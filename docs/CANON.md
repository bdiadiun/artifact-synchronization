...
| [A-29](decisions/A-29-channel-is-transport.md)            | 2026-09-22 | The channel is transport only (origin, version, guard, typed `send`, one handler map, queue and `exchange` on the host end); the bridge keeps the armed row, announces once by unsubscribing, answers with a plain `send` carrying `causedBy`. Three approved behaviour changes: OHIF's own `MEASUREMENT_REMOVED` is no longer suppressed on a form-driven removal; `activate` sends no `DEACTIVATE_TOOL` for the previous row; every `VIEWER_READY` restores the current rows. Supersedes part of A-23. | approved 2026-09-22                                                                                                                                                                                                                                           |
| [A-30](decisions/A-30-no-request-answer.md)               | 2026-09-22 | Commands carry no `requestId`, events no `causedBy`; the one correlation is `rowId` (host) ↔ `measurementUid` (viewer), carried together by `MEASUREMENT_ADDED`. No echo loop by one rule per side: the form never sends in reaction to an event, the extension never sends in reaction to a command. `exchange`, pending answers, the answer table and the timeout are removed; `MEASUREMENTS_RESTORED` is an ordinary event. Supersedes A-10 and A-21, narrows A-8.                                    | approved 2026-09-22                                                                                                                                                                                                                                           |
| [A-31](decisions/A-31-one-channel.md)                     | 2026-09-22 | One `createChannel` for both ends (`send`, `onMessage`, `getState`, `subscribe`, `dispose`; `accept` = the contract guard for what this end receives, `readyOn` = the message that opens the way out and flushes the queue). No incoming queue of our own: the browser's task queue is the queue, bursts are throttled at the source. The armed-row cancel on unmount is the application's (`useHostChannel`). Supersedes A-22's two factories.                                                          | approved 2026-09-22                                                                                                                                                                                                                                           |
| [A-32](decisions/A-32-react-bridge.md)                    | 2026-09-22 | The bridge is a React layer: the extension registers a context module whose provider OHIF mounts around the mode; `useScoringBridge` owns the channel, the commands, the measurement subscriptions and the announcement as effects with cleanups (Q-5). The channel package offers `useChannelState`; subscribing is an effect written where it happens. No `preRegistration`, no disposer list, no `pagehide`.                                                                                          | approved 2026-09-22                                                                                                                                                                                                                                           |
