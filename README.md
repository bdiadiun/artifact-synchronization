...
- Layout: `host-app/` (React + Vite, port 5173) and four published packages: `packages/contract/`
  (the message schemas, zod), `packages/channel/` (one `createChannel` for both sides: origin check, version,
  schema guard, `send`, `onMessage`, and the queue until `VIEWER_READY`),
