...
| Concern                   | File                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Viewer side of the bridge | `packages/viewer-bridge/src/extension.ts` (the OHIF extension: reads the host origin, creates the viewer channel, announces `VIEWER_READY` on `VIEWPORT_ADDED`, disposes on `pagehide`), `commands.ts` (the five command handlers as one `MessageHandlers` map), `measurements.ts` (OHIF measurement → `metrics` and `geometry`, the three subscriptions), `restore.ts` (S-5.6: readiness gate, re-adding annotations), `throttle.ts`, `ohif.ts` (the OHIF surface we call, the version overlay). What is protocol — announcing, the armed row, replying to a command — is the channel's (A-23) |
...
| Host side of the bridge   | `createHostChannel` from the channel package (A-22): holds commands until `VIEWER_READY` and cancels the armed row on unmount. The React bindings stay in the form: `host-app/src/hooks/useHostChannel.ts` (created and disposed in a layout effect, so the cancelling command still finds the iframe) and `useChannelState.ts` (`useSyncExternalStore` over `ready` and `queued`)                                                                                                                                                                                                              |
| Form rows and commands    | `host-app/src/form/rows.ts` (pure reducer delegating to one helper per action), `selectors.ts`, `commands.ts` (command builders), `rowActions.ts` (user actions), `viewerEventHandlers.ts` (one handler per viewer event, registered once with `onEach`), `hooks/useScoringForm.ts` (the reducer and the subscription), `format.ts` (row status, kind and value)                                                                                                                                                                                                                                |
