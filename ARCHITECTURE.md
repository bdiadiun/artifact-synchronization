...
| Form rows and commands    | `host-app/src/form/rows.ts` (the row types, the pure reducer delegating to one helper per action, the row lookups), `rowActions.ts` (user actions), `viewerEventHandlers.ts` (one `switch` over the viewer events, registered once with `onMessage`), `useScoringForm.ts` (the reducer and the subscription), `storage.ts` (A-14: the stored-row schema, `loadStoredRows` and `saveRows`; `useScoringForm` loads once and saves on every change), `format.ts` (row status, kind and value)                                                                                                                                               |
