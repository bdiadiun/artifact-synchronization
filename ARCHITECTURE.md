...
| viewer → host | `MEASUREMENTS_RESTORED` | `causedBy?`, `restored[]` (row ids), `failed[]` (`rowId`, `reason`)                                      | Answer to the restore command. Reasons: `already-present`, `unknown-study`, `invalid-geometry`, `viewer-error`. Bonus S-5.6.                                                                                                                                                    |
...
| Host side of the bridge   | The published package `@bdiadiun/scoring-orchestrator` (A-17): `createOrchestrator.ts` (wiring), `messageHandler.ts` (origin check, guard, dispatch), `commandQueue.ts` (early commands), `listeners.ts`, `commands.ts` (command builders). The React binding `host-app/src/hooks/useBridge.ts` stays in the form                                                                                                                                                                                                                                            |
...
| Origins and study link    | `host-app/src/config.ts`: the origins, and the study resolved once per page load from the form's own `study` parameter with the previous constant as fallback (A-19); `viewer/extensions/scoring-bridge/src/config.ts`                                                                                                                                                                                                                                                                                                                                       |
