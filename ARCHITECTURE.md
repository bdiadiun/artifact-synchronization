...
| host → viewer | `DEACTIVATE_TOOL`       | `requestId`, `rowId`                                                                                     | Cancel clicked, or another row activated. The viewer returns to its default tool (A-23).                                                                                                                                                                                                                                       |
...
| Handshake       | The host end of the channel keeps `ready` + FIFO queue (A-22); commands before `VIEWER_READY` are queued and flushed in order, never dropped. A second `VIEWER_READY` (reload) re-arms the row in `Drawing…`.                                                                                                                                              | A-9             |
...
| Layers          | Contract (wire format) → channel (the whole communication API: post, queue, exchange, state) → the two applications, which register one handler map with `onEach`. No orchestrator package, no `lastEvent`: an event goes channel → handler → reducer (A-22).                                                                                              |
| Minimal bridge  | The viewer extension holds only what is about OHIF: activating a tool, reading `cachedStats`, rebuilding an annotation. Announcing readiness, the armed row and `reply(command, …)` live in the channel's viewer end (A-23).                                                                                                                               |
| Contract form   | Every message is one zod schema; its type is `z.infer` under the same name; the two directions are discriminated unions and the guards are `safeParse` (A-26). `zod` is the project's one library.                                                                                                                                                         |
| Folder layout   | A folder names a side or a role: channel `host/ viewer/ shared/`, extension `commands/ events/ ohif/`, app `channel/ form/ components/ pages/`; no file under twenty lines; `.props.ts` only beside a React component (A-27).                                                                                                                              |

## Where things are

| Concern                   | File                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Viewer side of the bridge | `packages/viewer-bridge/src/extension.ts` (the OHIF extension: reads the host origin, creates the viewer channel, announces `VIEWER_READY` on `VIEWPORT_ADDED`, disposes on `pagehide`), `commands/handlers.ts` (the five command handlers as one `MessageHandlers` map), `commands/restore.ts` (S-5.6: readiness gate, re-adding annotations), `events/measurements.ts` (OHIF measurement → `metrics` and `geometry`, the three subscriptions), `ohif/surface.ts` (the OHIF surface we call, the version overlay), `ohif/throttle.ts` (A-27). What is protocol — announcing, the armed row, replying to a command — is the channel's (A-23) |
...
| The channel, used by both | The published package `@bdiadiun/scoring-channel` (A-21): the origin check, the contract guard, posting with an explicit target origin, and `send`, `onEach` and `exchange` typed from the message type. It fills in `version` and the request id, rejects an exchange whose answer never arrives, and owns the way out (A-22): the queue that holds messages until the peer's ready message, flushed in order before the application's handlers run, and, on the host end, the observable `{ ready, queued }`. `createHostChannel` and `createViewerChannel` are the two ends                                                               |
| Host side of the bridge   | `createHostChannel` from the channel package (A-22): holds commands until `VIEWER_READY` and cancels the armed row on unmount. The React bindings stay in the app: `host-app/src/channel/useHostChannel.ts` holds `useHostChannel` (created and disposed in a layout effect, so the cancelling command still finds the iframe) and `useChannelState` (`useSyncExternalStore` over `ready` and `queued`)                                                                                                                                                                                                                                      |
| Form rows and commands    | `host-app/src/form/rows.ts` (the row types, the pure reducer delegating to one helper per action, the row lookups), `rowActions.ts` (user actions), `viewerEventHandlers.ts` (one handler per viewer event, registered once with `onEach`), `useScoringForm.ts` (the reducer and the subscription), `storage.ts` (A-14: the stored-row schema and the two hooks around it), `format.ts` (row status, kind and value)                                                                                                                                                                                                                         |
