...
| The channel, used by both | The published package `@bdiadiun/scoring-channel` (A-21): the origin check, the contract guard, posting with an explicit target origin, and `send`, `on`, `onEach` and `exchange` typed from the message type. It fills in `version` and the request id, rejects an exchange whose answer never arrives, and owns the way out (A-22): the queue that holds messages until the peer's ready message, flushed in order before the application's handlers run, and the observable `{ ready, queued }`. `createHostChannel` and `createViewerChannel` are the two ends                                                                           |
